    ...
    <div class="wrap">
      ...
      <table class="table" style="margin-bottom:40px">
        <el-table :data="list" border style="width:100%">
          <el-table-column label="商品名称" width="720">
            <template slot-scope="scope">
              <div style="margin-left:50px">
                <img :src="scope.row.GoodsPic" style="height:50px;width:50px" />
                <span style="font-size:18px;padding-left:200px;">{{scope.row.GoodsName}}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="数量" align="center" prop="number" width="192">
            <template slot-scope="scope">
              <span>{{scope.row.CarSum}}</span>
            </template>
          </el-table-column>
          <el-table-column label="支付金额" align="center" prop="price" width="288">
            <template slot-scope="scope">
              <span>{{scope.row.CarSum * scope.row.Price}}</span>
            </template>
          </el-table-column>
        </el-table>
      </table>
...
export default {
  ...
  created() {
    ...
    console.log(this.list + "：获取到购物车数据");
  },
  methods: {
    getvalue() {
  ...
  computed: {
    moneyTotal() {
      ...
      for (var i = 0; i < list.length; i++) {
        total += parseFloat(list[i].Price) * list[i].CarSum;
      }
